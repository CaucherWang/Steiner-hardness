...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
   public:
    ...
    std::unordered_map<labeltype, tableint>
        label_lookup_;  // external label -> internal id
    ...
    searchBaseLayer(tableint ep_id, const void *data_point, int layer) {
        ...
        // top_candidates; std::priority_queue<std::pair<dist_t, tableint>,
        // std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        // candidateSet;
...
//                    if (candidate_id == 0) continue;
    ...
    std::priority_queue<std::pair<dist_t, tableint>,
                        std::vector<std::pair<dist_t, tableint>>>
    searchBaseLayerST_DEEP_QUERY(
        tableint ep_id, const void *data_point, size_t ef, size_t k,
...
#else
            dist_t dist =
                hnswlib::L2Sqr_by_IP(data_point, getDataByInternalId(ep_id),
                                     getExternalLabel(ep_id), dist_func_param_);
#endif
        ...
        cout << "#hop\tInd\t\tOutd\tDist2Q\tLB\t\tBSF\t\tparent\tDist2P\t\tid\t"
                "label\trecall\tNDC"
             << endl;

        // Iteratively generate candidates and conduct DCOs to maintain the
        // result set R.
        while (!candidate_set.empty()) {
            std::pair<dist_t, tableint> current_node_pair = candidate_set.top();

            // When the smallest object in S has its distance larger than the
            // largest in R, terminate the algorithm.
            ...
            cout << cur_hop << "\t\t" << indegrees_[current_node_id] << "\t\t"
                 << size << "    \t" << -current_node_pair.first << "\t"
                 << lowerBound << "\t" << topk_dist << "\t"
                 << parent_map[current_node_id] << "\t"
                 << fstdistfunc_(
                        getDataByInternalId(current_node_id),
                        getDataByInternalId(parent_map[current_node_id]),
                        dist_func_param_)
                 << "\t" << current_node_id << "\t"
                 << getExternalLabel(current_node_id);

            // Enumerate all the neighbors of the object and view them as
            // candidates of KNNs.
            for (size_t j = 1; j <= size; j++) {
                ...
                if (!(visited_array[candidate_id] == visited_array_tag)) {
                    ...
                    // (2) compare it with the N_ef th distance (i.e.,
                    // lowerBound)
                    char *currObj1 = (getDataByInternalId(candidate_id));
#ifdef COUNT_DIST_TIME
                    StopW stopw = StopW();
                    ...
                    dist_t dist =
                        fstdistfunc_(data_point, currObj1, dist_func_param_);
#else
                    dist_t dist = hnswlib::L2Sqr_by_IP(
                        data_point, currObj1, getExternalLabel(candidate_id),
                        dist_func_param_);
#endif
#ifdef COUNT_DIST_TIME
                    adsampling::distance_time += stopw.getElapsedTimeMicro();
                    ...
                    if (top_candidates.size() < ef || lowerBound > dist) {
                        ...
                        if (!top_candidates.empty())
                            lowerBound = top_candidates.top().first;
                        if (!topk.empty()) topk_dist = topk.top().first;
                    }
                }
            }
    ...
    std::priority_queue<std::pair<dist_t, tableint>,
                        std::vector<std::pair<dist_t, tableint>>>
    searchBaseLayerST_STAT_QUERY(tableint ep_id, const void *data_point,
        ...
        while (!candidate_set.empty()) {
            ...
            hop_distance.push_back(
                fstdistfunc_(getDataByInternalId(current_node_id),
                             getDataByInternalId(parent_map[current_node_id]),
                             dist_func_param_));
    ...
    searchBaseLayerST(tableint ep_id, const void *data_point, size_t ef,
                      int ndc_upperbound, Metric *metric = nullptr) const {
        VisitedList *vl = visited_list_pool_->getFreeVisitedList();
        vl_type *visited_array = vl->mass;
        vl_type visited_array_tag = vl->curV;
    ...
    std::priority_queue<std::pair<dist_t, tableint>,
                        std::vector<std::pair<dist_t, tableint>>>
    searchBaseLayernStep(tableint ep_id, const void *data_point, size_t nstep,
                         size_t k, int ndc_upperbound,
                         Metric *metric = nullptr) const {
        VisitedList *vl = visited_list_pool_->getFreeVisitedList();
        vl_type *visited_array = vl->mass;
        vl_type visited_array_tag = vl->curV;
        ...
        while (!candidate_set.empty()) {
            ...
            // largest in R, terminate the algorithm.
            if ((-current_node_pair.first) > lowerBound &&
                (top_candidates.size() == k || has_deletions == false)) {
                break;
            }
            candidate_set.pop();
            adsampling::tot_hops++;
        ...
        tableint cur_c = 0;
        {
            ...
            if (search != label_lookup_.end()) {
                ...
                std::unique_lock<std::mutex> lock_el_update(
                    link_list_update_locks_[(existingInternalId &
                                             (max_update_element_locks - 1))]);
    ...
    tableint addPointPlain(const void *data_point, labeltype label,
                           int level = -1) {
        tableint cur_c = 0;
        {
            // Checking if the element with the same label already exists
    ...
    tableint addPointPlain_with_id(const void *data_point, labeltype label,
                                   int internal_id) {
    ...
    std::priority_queue<std::pair<dist_t, labeltype>> searchKnn(
        void *query_data, size_t k, int adaptive = 0) const {
        ...
        dist_t curdist =
            fstdistfunc_(query_data, getDataByInternalId(enterpoint_node_),
                         dist_func_param_);
#else
        dist_t curdist = hnswlib::L2Sqr_by_IP(
            query_data, getDataByInternalId(enterpoint_node_),
            getExternalLabel(enterpoint_node_), dist_func_param_);
#endif
#ifdef COUNT_DIST_TIME
        adsampling::distance_time += stopw.getElapsedTimeMicro();
#endif
                    ...
                    if (cand < 0 || cand > max_elements_)
                        ...
                        dist_t d =
                            fstdistfunc_(query_data, getDataByInternalId(cand),
                                         dist_func_param_);
#else
                        dist_t d = hnswlib::L2Sqr_by_IP(
                            query_data, getDataByInternalId(cand),
                            getExternalLabel(cand), dist_func_param_);
#endif
#ifdef COUNT_DIST_TIME
                        adsampling::distance_time +=
                            stopw.getElapsedTimeMicro();
#endif
        ...
        if (num_deleted_) {
            ...
            top_candidates = searchBaseLayerST<false, true>(
                currObj, query_data, std::max(ef_, k), 0);
#elif defined(STAT_QUERY)
            top_candidates = searchBaseLayerST_STAT_QUERY<false, true>(
                currObj, query_data, std::max(ef_, k), k);
#else
            ...
            top_candidates = searchBaseLayerST_DEEP_QUERY<false, true>(
                currObj, query_data, std::max(ef_, k), k, gt);
#endif
        }
    ...
    // plain
    std::priority_queue<std::pair<dist_t, labeltype>> searchKnnPlain(
        void *query_data, size_t k, int ndc_upperbound, int adaptive = 0,
        Metric *metric = nullptr, int ef = 0) const {
...
#ifndef ED2IP
        dist_t curdist = fstdistfunc_(
            query_data, getDataByInternalId(entry_point), dist_func_param_);
#else
        dist_t curdist = hnswlib::L2Sqr_by_IP(
            query_data, getDataByInternalId(entry_point),
            getExternalLabel(entry_point), dist_func_param_);
#endif
#ifdef COUNT_DIST_TIME
        adsampling::distance_time += stopw.getElapsedTimeMicro();
#endif
        ...
        if (num_deleted_) {
            top_candidates = searchBaseLayerST<true, true>(
                currObj, query_data, std::max(ef_, k), ndc_upperbound, metric);
        } else {
            if (use_ef >= k)
                top_candidates = searchBaseLayerST<false, true>(
                    currObj, query_data, std::max(use_ef, k), ndc_upperbound,
                    metric);
            else
                top_candidates = searchBaseLayernStep<false, true>(
                    currObj, query_data, use_ef, k, ndc_upperbound, metric);
        }
...
#ifdef DEEP_QUERY
    // plain
    std::priority_queue<std::pair<dist_t, labeltype>> searchKnnPlainDEEP_QUERY(
        void *query_data, size_t k,
        std::priority_queue<std::pair<dist_t, labeltype>> &gt_input) const {
        std::priority_queue<std::pair<dist_t, labeltype>> result;
        if (cur_element_count == 0) return result;
    ...
    void getDegrees(vector<int> &out_degree, vector<int> &in_degree) {
        for (int i = 0; i < cur_element_count; i++) {
            ...
            size_t size = getListCount((linklistsizeint *)data);
            out_degree[i] = size;
            for (int j = 1; j <= size; ++j) in_degree[data[j]]++;
        }
    }
    ...
    void printGraph() {
        for (int i = 0; i < cur_element_count; i++) {
            ...
            size_t size = getListCount((linklistsizeint *)data);
            cout << i << ": ";
            for (int j = 1; j <= size; ++j) cout << data[j] << ",";
            cout << endl;
        }
    }
};
