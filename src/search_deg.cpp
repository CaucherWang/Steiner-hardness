...
template<typename data_t, typename dist_t>
static void test_vs_recall(data_t *massQ, size_t vecsize, size_t qsize,  DEG *appr_alg, size_t vecdim,
    ...
    // vector<size_t> efs{800, 1000, 1500, 2000, 2500, 3000};
...
template<typename data_t, typename dist_t>
static void test_performance(data_t *massQ, size_t vecsize, size_t qsize, DEG *appr_alg, size_t vecdim,
    ...
    for(int i = 0; i < qsize; ++i){
        ...
        for(int _ = 0; _ < 1 && !flag; ++_){
            ...
            while(lowef < highef){
                ...
                metric.clear();
                auto result = appr_alg->searchDEG(massQ + vecdim * i, k, curef, metric);  
            ...
            //     tmp = recall(result, gt);
            //     if(tmp >= lowk){
            //         std::cout << adsampling::tot_full_dist << ",";
            //     }else{
            //         std::cout << large_ndc << ",";
            //     }
            //     flag = true;
...
int main(int argc, char * argv[]) {
    ...
    while(iarg != -1){
        iarg = getopt_long(argc, argv, "d:r:k:g:s:", longopts, &ind);
        switch (iarg){
            case 'd':
                if(optarg) data_str = string(optarg);
                break;
            case 'g':
                if(optarg) degree = atoi(optarg);
                break;
            case 'r':
            ...
            case 'k':
                ...
                break;
            case 's':
                if(optarg) shuf_postfix = string(optarg);
                break;
        }
    }
    ...
    result_path_str = result_base_path_str + "/" + data_str + "/" + result_prefix_str + data_str + "_DEG_k" + to_string(subk) + "_d" + to_string(degree) + "_" + exp_name + ".log" + index_postfix + shuf_postfix + query_postfix;
    #ifdef DEEP_DIVE
    ...
    std::cout << "result path: "<< result_path << endl;
    int simd_lowdim = -1;

    // adsampling::D = Q.d;
    freopen(result_path,"a",stdout);
    std::cout << "k: "<<subk << endl;;
    std::cerr << "ground truth path: " << groundtruth_path << endl;
    Matrix<unsigned> G(groundtruth_path);
    size_t k = G.d;
    unsigned q_num = 100000;

    if(data_type == 0){
        std::cerr << "query path: " << query_path << endl;
        Matrix<float> Q(query_path);
        
        Q.n = Q.n > q_num ? q_num : Q.n;
        std::cerr << "Query number = " << Q.n << endl;

        L2Space space(Q.d);   
        std::cerr << "L2 space" << endl;
        std::cerr << "Read index from " << index_path << endl;
        Matrix<float>X(data_path);

        DEG *deg;
        if(data_str == "deep")
            deg = new DEG(index_path, X.data, &space, true);
        else
            deg = new DEG(index_path, X.data, &space, false);
