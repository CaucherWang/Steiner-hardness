...
def generate_paths(data_directory, dataset_name, params, postfix):
    ...
    paths = {
        ...
        'model_path': os.path.join(base_path, f'{dataset_name}_gmm.bin'),
...
#     write_int_file(groundtruth_path, gt_indices)
...
def generate_workload(paths, workloads_size, postfix):
...
def generate_data_with_GMM(paths, n_components, n_samples, postfix):
...
def preprocess_dataset(
    ...
    run_file(os.path.join(efanna_directory, 'tests'), 'test_nndescent', [paths['data_path'], paths['kgraph_path'], params['efanna_K'], params['efanna_L'], params['efanna_iter'], params['efanna_S'], params['efanna_R']])

    print('Building mrng index...')
    compile_file(source_directory, 'index_mrng.cpp', 'index_mrng', ['-ffast-math', '-march=native'])
    
    run_file(source_directory, 'index_mrng', ['-d', paths['data_path'], '-i', paths['mrng_path'], '-g', paths['kgraph_path'], '-k', params['kod'], '-m', 0])

    print('Building reversed mrng index...')
    reverse_mrng_graph(paths)

    print('Generating benchmark with GMM...')
    generate_data_with_GMM(paths, params['n_components'], params['n_samples'], postfix)
    

# 计算hardness
# {'k': 100, 'recall': 0.99, 'prob': 0.99, 'method': 0}
def calculate_hardness(paths, data_directory, source_directory, dataset_name, params):
    print('Computing groundtruth...')
    compile_file(source_directory, 'compute_gt.cpp', 'compute_gt', [])
    run_file(source_directory, 'compute_gt', ['-d', paths['data_path'], '-q', paths['query_path'], '-g', paths['groundtruth_path'], '-k', params['k'], '-n', params['groundtruth_num']])
...
def build_benchmark(source_directory, data_directory, paths, dataset_name, params, postfix):
    ...
    benchmark_groundth_path = os.path.join(data_directory, dataset_name, f'{dataset_name}_benchmark_groundtruth.i{postfix}')

    compile_file(source_directory, 'compute_gt.cpp', 'compute_gt', [])
    run_file(source_directory, 'compute_gt', ['-d', paths['data_path'], '-q', paths['benchmark_path'], '-g', benchmark_groundth_path, '-k', params['k'], '-n', params['groundtruth_num']])
    ...
    for recall in params['recalls']:
        for prob in params['probs']:
            paths['workload_path'] = os.path.join(dataset_path, f'{dataset_name}_benchmark_recall{recall:.2f}.f{postfix}')
            paths['workload_groundtruth_path'] = os.path.join(dataset_path, f'{dataset_name}_benchmark_groundtruth_recall{recall:.2f}.i{postfix}')
            paths['hardness_path'] = os.path.join(dataset_path, f"{dataset_name}_hardness_recall{recall:.2f}_prob{prob:.2f}_k{params['k']}_K{params['KMRNG']:}.ibin_mrng_benchmark")
...
def main():
    ...
    def run_preprocess():
        ...
        params.update({
            'efanna_K': preprocess_config['efanna_K'],
            ...
            'k': preprocess_config['k'],
            'kod': preprocess_config['kod'],
            'n_components': preprocess_config['n_components'],
            'n_samples': preprocess_config['n_samples'],
            'KMRNG': preprocess_config['KMRNG']
        })
        paths.update(generate_paths(data_directory, preprocess_config['dataset_name'], params, preprocess_config['postfix']))
        preprocess_dataset(preprocess_config['efanna_directory'], source_directory, paths, params, preprocess_config['postfix'])
    ...
    def run_hardness():
        ...
        params.update({
            ...
            'groundtruth_num': hardness_config['groundtruth_num'],
            'KMRNG': hardness_config['KMRNG']
        })
        paths.update(generate_paths(data_directory, hardness_config['dataset_name'], params, hardness_config['postfix']))
        calculate_hardness(paths, data_directory, source_directory, hardness_config['dataset_name'], params)
    ...
    def run_benchmark():
        ...
        params.update({
            ...
            'groundtruth_num': benchmark_config['groundtruth_num'],
            'recalls': benchmark_config['recalls'],
            'probs': benchmark_config['probs'],
            'workloads_size': benchmark_config['workloads_size'],
            'KMRNG': benchmark_config['KMRNG']
        })
        paths.update(generate_paths(data_directory, benchmark_config['dataset_name'], params, benchmark_config['postfix']))
        build_benchmark(source_directory, data_directory, paths, benchmark_config['dataset_name'], params, benchmark_config['postfix'])
